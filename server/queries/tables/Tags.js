...
class Tags {
    list = async () => { return (await new Builder(`tbl_tags`).select().condition(`ORDER BY date_created DESC`).build()).rows; }
    specific = async (id) => { return (await new Builder(`tbl_tags`).select().condition(`WHERE id= ${id}`).build()).rows; }
    dropdown = async () => { return (await new Builder(`tbl_tags`).select(`id, name`).condition(`WHERE status= 1 ORDER BY name ASC`).build()).rows; }

    search = async (data) => { 
        return (await new Builder(`tbl_tags`)
                                        .select()
        ...
        if(!(await new Builder(`tbl_tags`).select().condition(`WHERE name= '${(data.name).toUpperCase()}'`).build()).rowCount > 0) {
            await new Builder(`tbl_tags`)
                                .insert({ columns: `series_no, name, status, created_by, date_created`, 
                                            values: `'${(data.series_no).toUpperCase()}', '${(data.name).toUpperCase()}', ${data.status ? 1 : 0}, ${data.created_by}, CURRENT_TIMESTAMP` }).build();
