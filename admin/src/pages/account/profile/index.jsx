...
const Index = () => {
    ...
    const { setValidation, handleSubmit, setValue, setError, register, getValues } = useContext(FormCntxt);
    const { refetch, isFetching } = 
        useGet({ key: ['usr_specific'], fetch: specific({ table: 'tbl_users', id: id }), options: { enabled: type !== 'new', refetchOnWindowFocus: false },
            onSuccess: (data) => {
    ...
    return (
        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ width: '100%', height: '100%', paddingBottom: '20px'}} spacing= { 3 }>
            <Stack direction= "row" justifyContent= "space-between" alignItems= "center">
                <Typography variant= "h6" sx= {{ fontFamily: 'Boldstrom', color: '#3C4048' }}>Profile</Typography>
            </Stack>
            <Box sx= { card }>
                ...
                <form autoComplete= "off">
                    <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" divider= { <Divider orientation="horizontal" flexItem /> } spacing= { 2 }>
                        ...
                        { getValues()?.user_level !== 'superadmin' ? 
                            <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch">
                ...
                <Grid container direction= "row" justifyContent= "flex-end" alignItems= "center">
                    <Grid item xs= { 12 } sm= { 3 } lg= { 2 }>
                        <Box sx= { btntxt } onClick= { handleSubmit(data => {
                            ...
                            if(data.avatar !== undefined) { updating({ table: 'tbl_users', data: data }); }
