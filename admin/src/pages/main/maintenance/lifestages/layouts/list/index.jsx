...
const Index = () => {
    ...
    const { isFetching: fetching } = useGet({ key: ['stages_list'], fetch: records({ table: 'tbl_life_stages', data: {} }), options: { refetchOnWindowFocus: false }, onSuccess: (data) => setList(data) });
                    ...
                    <form autoComplete= "off">
                        <Box sx= { search }>
                            ...
                            <TextField variant= "standard" size= "small" fullWidth InputProps= {{ disableUnderline: true }} placeholder= "Search..." sx= {{ padding: '5px 0 0 0' }}
                                onChange= { e => { find({ table: 'tbl_life_stages', data: { condition: e.target.value !== '' ? (e.target.value).toUpperCase() : e.target.value } }); } } />
                        </Box>
                    </form>
                    <Box>
                        <Typography component= { Link } to= "/maintenance/lifestages/form/new" sx= { btnicon }><FontAwesomeIcon icon= { solid('plus') } style= {{ color: '#FFFFFF' }} /></Typography>
                        <Typography component= { Link } to= "/maintenance/lifestages/form/new" sx= { btntxt }>New Life Stages</Typography>
                    </Box>
                </Stack>
            </Stack>
