...
const Index = () => {
    ...
    const { isFetching: fetching } = useGet({ key: ['coat_list'], fetch: records({ table: 'tbl_coat', data: {} }), options: { refetchOnWindowFocus: false }, onSuccess: (data) => setList(data) });

    return (
        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ width: '100%', height: '100%' }} spacing= { 3 }>
            <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" spacing= { 1 }>
                <Typography variant= "h6" sx= {{ fontFamily: 'Boldstrom', color: '#3C4048' }}>Coat</Typography>
                    ...
                    <form autoComplete= "off">
                        <Box sx= { search }>
                            ...
                            <TextField variant= "standard" size= "small" fullWidth InputProps= {{ disableUnderline: true }} placeholder= "Search..." sx= {{ padding: '5px 0 0 0' }}
                                onChange= { e => { find({ table: 'tbl_coat', data: { condition: e.target.value !== '' ? (e.target.value).toUpperCase() : e.target.value } }); } } />
                        </Box>
                    </form>
                    <Box>
                        <Typography component= { Link } to= "/maintenance/coat/form/new" sx= { btnicon }><FontAwesomeIcon icon= { solid('plus') } style= {{ color: '#FFFFFF' }} /></Typography>
                        <Typography component= { Link } to= "/maintenance/coat/form/new" sx= { btntxt }>New Coat</Typography>
                    </Box>
                </Stack>
            </Stack>
