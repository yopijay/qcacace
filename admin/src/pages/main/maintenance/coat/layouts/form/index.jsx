...
const Index = () => {
    ...
    const { refetch, isFetching: fetching } = 
        useGet({ key: ['coat_specific'], fetch: specific({ table: 'tbl_coat', id: id ?? null }), options: { enabled: type !== 'new', refetchOnWindowFocus: false },
            onSuccess: (data) => { 
    ...
    const { mutate: saving } = 
        usePost({ fetch: save, 
            onSuccess: (data) => { 
                ...
                else { successToast(data.message, 3000, navigate('/maintenance/coat', { replace: true })); } 
            } 
        });
    ...
    return (
        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ width: '100%', height: '100%', paddingBottom: '20px' }} spacing= { 3 }>
            <Stack direction= "row" justifyContent= "space-between" alignItems= "center">
                ...
                <Typography sx= { btnicon } component= { Link } to= "/maintenance/coat"><FontAwesomeIcon icon= { solid('chevron-left') }/></Typography>
            </Stack>
            <Box sx= { card }>
                    ...
                    <Grid item xs= { 12 } sm= { 3 } lg= { 2 }>
                        <Box sx= { btntxt } onClick= { handleSubmit(data => {
                            ...
                            if(data.category_id !== 0) {
                                if(type === 'new') { saving({ table: 'tbl_coat', data: data }); }
                                else { updating({ table: 'tbl_coat', data: data }); }
                            }
