...
const Index = () => {
    ...
    const { refetch, isFetching: fetching } = useGet({ key: ['tag_specific'], fetch: specific({ table: 'tbl_pet_tags', id: id ?? null }), options: { enabled: type !== 'new', refetchOnWindowFocus: false },
        onSuccess: (data) => { 
    ...
    const { mutate: saving } = usePost({ fetch: save, 
        onSuccess: (data) => { 
            ...
            else { successToast(data.message, 3000, navigate('/maintenance/tags', { replace: true })); } 
        } 
    });
    ...
    return (
        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ width: '100%', height: '100%', paddingBottom: '20px' }} spacing= { 3 }>
            <Stack direction= "row" justifyContent= "space-between" alignItems= "center">
                ...
                <Typography sx= { btnicon } component= { Link } to= "/maintenance/tags" ><FontAwesomeIcon icon= { solid('chevron-left') }/></Typography>
            </Stack>
            <Box sx= { card }>
