...
const Dashboard = () => {
    const { data: count, isFetching } = useGet({ key: ['pets_dashboard'], fetch: dashboard('tbl_pets') });
        ...
        <Grid container direction= "row" justifyContent= "flex-start" alignItems= 'stretch' sx= {{ padding: '5px 0' }}>
            ...
            <Grid item xs= { 3 } sm= { 2 } sx= {{ padding: { xs: '0 0 0 5px' } }}>
                <Stack direction= "column" justifyContent= "flex-start" alignItems= "flex-start" 
                    ...
                    { !isFetching ? <Typography variant= "h5" sx= {{ fontFamily: 'Boldstrom', color: '#2c3e50' }}>{ `${count.adopted < 10 ? '0' : ''}${count.adopted}` }</Typography> : 
                        <Skeleton variant= "text" sx= {{ width: '50px', fontSize: '1rem' }} /> }
                    <Typography variant= "body1" 
                        sx= {{ fontWeight: 'bold', color: '#34495e', width: '100%', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Adopted</Typography>
                </Stack>
            </Grid>
        </Grid>
    );
