...
const Item = () => {
    ...
    return (
        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" spacing= { 1 }>
            { list?.length > 0 ?
                list?.map((data, index) => (
                    <Stack direction= "row" justifyContent= "sapce-between" alignItems= "center" key= { index } sx= { item } spacing= { 2 }>
                        <Avatar />
                        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" flexGrow= "1">
                            ...
                            <Typography variant= "body1" sx= {{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ data.age }, { data.size }</Typography>
                        ...
                        <Stack direction= "row" justifyContent= "flex-start" alignItems= "center" spacing= { 2 }>
                            <Typography sx= { icons } component= { Link } to= { `/maintenance/pets/form/update/${data.id}` }><FontAwesomeIcon icon= { solid('pencil') } size= "lg" /></Typography>
