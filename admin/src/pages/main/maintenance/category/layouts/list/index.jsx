...
const Index = () => {
    ...
    const { isFetching } = useGet(['ctg_list'], record({ table: 'tbl_pet_category', query: `ORDER BY date_created DESC` }), { refetchOnWindowFocus: false }, (data) => setList(data));

    return (
        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ width: '100%', height: '100%' }} spacing= { 1 }>
            <Typography sx= { title }>Pet Category list</Typography>
            <Stack direction= "row" justifyContent= "space-between" alignItems= "center">
                ...
                <Box>
                    <Typography component= { Link } to= "/maintenance/category/form/new" sx= { btnicon }><FontAwesomeIcon icon= { solid('plus') } style= {{ color: '#ffffff' }} /></Typography>
                    <Typography component= { Link } to= "/maintenance/category/form/new" sx= { btntxt }>New Pet Category</Typography>
                </Box>
            </Stack>
