...
const Dashboard = () => {
    const { data: count, isFetching } = useGet({ key: ['pymnt_dashboard'], fetch: dashboard('tbl_payments') });
        ...
        <Grid container direction= "row" justifyContent= "flex-start" alignItems= 'stretch' sx= {{ padding: '5px 0' }}>
            ...
            <Grid item xs= { 3 } sm= { 2 } sx= {{ padding: { xs: '0 5px' } }}>
                <Stack direction= "column" justifyContent= "flex-start" alignItems= "flex-start" 
                    ...
                    { !isFetching ? <Typography variant= "h5" sx= {{ fontFamily: 'Boldstrom', color: '#2c3e50' }}>{ `${count.paid < 10 ? '0' : ''}${count.paid}` }</Typography> : 
                        <Skeleton variant= "text" sx= {{ width: '50px', fontSize: '1rem' }} /> }
                    <Typography variant= "body1" 
                        sx= {{ fontWeight: 'bold', color: '#34495e', width: '100%', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Paid</Typography>
                </Stack>
            </Grid>
