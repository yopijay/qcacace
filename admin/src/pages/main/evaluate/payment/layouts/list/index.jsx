...
const Index = () => {
    ...
    const { isFetching: fetching } = 
        useGet({ key: ['pymnt_list'], fetch: records({ table: 'tbl_payments', data: {} }), options: { refetchOnWindowFocus: false }, onSuccess: data => setList(data) });
                    ...
                    <form autoComplete= "off">
                        <Box sx= { search }>
                            ...
                            <TextField variant= "standard" size= "small" fullWidth InputProps= {{ disableUnderline: true }} placeholder= "Search..." sx= {{ padding: '5px 0 0 0' }}
                                onChange= { e => { find({ table: 'tbl_payments', data: { condition: e.target.value !== '' ? (e.target.value).toUpperCase() : e.target.value } }); } } />
                        </Box>
                    </form>
                </Stack>
            </Stack>
