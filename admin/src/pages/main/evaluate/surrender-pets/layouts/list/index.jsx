...
const Index = () => {
    ...
    const { isFetching: fetching } = 
        useGet({ key: ['surr_list'], fetch: records({ table: 'tbl_surrender', data: {} }), options: { refetchOnWindowFocus: false }, onSuccess: data => setList(data) });

    return (
        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ width: '100%', height: '100%' }} spacing= { 3 }>
            <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" spacing= { 1 }>
                <Typography variant= "h6" sx= {{ fontFamily: 'Boldstrom', color: '#3C4048' }}>Surrendered Pets</Typography>
                    ...
                    <form autoComplete= "off">
                        <Box sx= { search }>
                            ...
                            <TextField variant= "standard" size= "small" fullWidth InputProps= {{ disableUnderline: true }} placeholder= "Search..." sx= {{ padding: '5px 0 0 0' }}
                                onChange= { e => { find({ table: 'tbl_surrender', data: { condition: e.target.value !== '' ? (e.target.value).toUpperCase() : e.target.value } }); } } />
                        </Box>
                    </form>
                </Stack>
            </Stack>
