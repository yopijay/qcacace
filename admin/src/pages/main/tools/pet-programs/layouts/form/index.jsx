...
const Index = () => {
    ...
    const { refetch, isFetching: fetching } = 
        useGet({ key: ['program_specific'], fetch: specific({ table: 'tbl_programs', id: id ?? null }), options: { enabled: type !== 'new', refetchOnWindowFocus: false },
            onSuccess: (data) => { 
                ...
                <Grid container direction= "row" justifyContent= "flex-end" alignItems= "center">
                    <Grid item xs= { 12 } sm= { 3 } lg= { 2 }>
                        <Box sx= { btntxt } onClick= { handleSubmit(data => {
                            ...
                            if(type === 'new') { saving({ table: 'tbl_programs', data: data }); }
                            else { updating({ table: 'tbl_programs', data: data }); }
                        }) }>Save</Box>
                    </Grid>
                </Grid> : '' }
        </Stack>
