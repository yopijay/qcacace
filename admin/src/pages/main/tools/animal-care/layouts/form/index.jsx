...
const Index = () => {
    ...
    const { mutate: removing } = usePost({ fetch: remove, onSuccess: (data) => { successToast(data.message, 3000, navigate('/tools/animalcare', { replace: true })); } });
        ...
        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ width: '100%', height: '100%', paddingBottom: '20px' }} spacing= { 3 }>
            <Stack direction= "row" justifyContent= "space-between" alignItems= "center">
                ...
                <Stack direction= "row" justifyContent= "flex-end" alignItems= "center" spacing= { 1 }>
                    ...
                    { type === 'update' ? <Typography sx= { btndelete } onClick= { () => removing({ table: 'tbl_animal_care', id: id }) }><FontAwesomeIcon icon= { solid('trash') }/></Typography> : '' }
                </Stack>
            </Stack>
